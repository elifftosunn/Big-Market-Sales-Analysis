...
# # Target'a Gore Kolonları Gruplama
...
# outlier value review of columns (sütunların aykırı deger incelenmesi)
for col in numeric_cols:
    print(col, ":", dataUnderstand(df).checkOutlier(col))

# capture of outliers as a single variable (Aykırı değerlerin tek değişkenli olarak yakalanması)
for col in numeric_cols:
    print(col, ":\n", dataUnderstand(
        df).catchOutliers(col, index=True, plot=True))

# crushing outliers (aykırı degerler baskılandı)
for col in numeric_cols:
...
#dataUnderstand(df).categoricVisualisation(categoric_cols, "Item_Outlet_Sales")  // BU KISMA BAKKKKKKKKKKKKKKK     
